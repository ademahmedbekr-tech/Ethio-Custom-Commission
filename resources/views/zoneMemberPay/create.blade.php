...
@section('model','Zone Member Payment')
@section('title','Zone Member Payment')
{{-- @section('back',route('{{ $model }}.index')) --}}
@section('type','Create')
@section('form')


<form method="POST" action="{{ route('zoneMemberPay.store') }}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="col-xs-12 col-sm-12 col-md-12">
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="woreda" name="woreda" value="{{ $member->woreda }}" readonly>
        </div>
    </div>
